...
// PAINEL DE AUTOMAÇÃO E TIMER
app.get('/painel', async (req, res) => {
    const { cpf } = req.query;
    const cpfLimpo = cpf.replace(/\D/g, ''); // Garante que busca apenas números
    
    // Busca a reserva paga mais recente para este CPF
    const reserva = await Reserva.findOne({ doc: cpfLimpo, status: 'pago' }).sort({ _id: -1 });
    ...
    <head>
        ...
        <style>
            ...
            .info { font-size: 0.7rem; color: #444; margin-top: 40px; text-transform: uppercase; letter-spacing: 2px; }
        </style>
    </head>
    <body>
        <h1>ŪNIKA</h1>
        ...
        <h2>ACESSOS</h2>
        <div class="grid">
            <button class="btn-iot" onclick="alert('Porta Masculina Liberada')">🚹 MASCULINO</button>
            <button class="btn-iot" onclick="alert('Porta Feminina Liberada')">🚺 FEMININO</button>
        </div>
        <div class="info">CLIENTE: ${reserva.nome}</div>
        ...
        <script>
            function startTimer(duration, dateStr, timeStr) {
                ...
                setInterval(() => {
                    ...
                    if (dist < 0) { display.innerHTML = "00:00"; return; }
