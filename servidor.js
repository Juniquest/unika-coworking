...
// 2. ROTA DO PAINEL DO CLIENTE (CONTROLES, TIMER E POP-UP BANHEIRO)
app.get('/painel', async (req, res) => {
    ...
    if (!reserva) {
        return res.send(`<body style="background:#050505;color:#d4af37;text-align:center;padding-top:100px;font-family:sans-serif;">
            <h1>ACESSO NEGADO</h1><p>Não encontramos uma reserva ativa para hoje.</p>
...
<head>
    ...
    <style>
        ...
        body { background: var(--bg); color: var(--text); font-family: 'Inter', sans-serif; margin: 0; display: flex; flex-direction: column; align-items: center; overflow-x: hidden; }
        ...
        .btn-bathroom.available { border-color: #28a745; color: #fff; background: rgba(40, 167, 69, 0.2); }
        #popup-banheiro { 
...
<body>
    ...
    <div class="container">
        ...
        <h2>Acessos</h2>
        <div class="control-grid">
            <button id="btn_banheiro" class="btn-iot btn-bathroom" onclick="abrirBanheiro()">🔒 Banheiro em Uso</button>
        </div>
    </div>
    <script>
        let banheiroLivreAnteriormente = false;
        function checarBanheiro() {
            let estaLivre = Math.random() > 0.5; // Simulação de sensor
        ...
        function toggle(btn, disp) { btn.classList.toggle('active'); }
        function abrirBanheiro() { if(document.getElementById('btn_banheiro').classList.contains('available')) alert("Porta Liberada!"); }
...
app.post('/webhook-asaas', async (req, res) => {
    ...
    if (event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_RECEIVED') {
        ...
        try {
            await Reserva.findOneAndUpdate({ doc: cpfCliente, status: 'pendente' }, { status: 'pago' }, { sort: { _id: -1 } });
...
<head>
    ...
    <style>
        ...
        .container { width: 92%; max-width: 500px; background: var(--card); border: 1px solid #222; padding: 30px; border-radius: 4px; }
        ...
        input { width: 100%; background: transparent; border: none; border-bottom: 2px solid #333; color: #fff; padding: 12px 0; font-size: 1.1rem; margin-bottom: 20px; }
        ...
        .service-item, .option-item { border: 1px solid #333; padding: 18px 5px; text-align: center; cursor: pointer; font-size: 0.8rem; font-weight: 600; }
        .service-item.selected, .option-item.selected { border-color: var(--gold); background: rgba(212, 175, 55, 0.2); }
        ...
        .hora-item { border: 1px solid #222; padding: 15px 2px; text-align: center; font-size: 1rem; cursor: pointer; font-weight: 600; }
...
<body>
    ...
    <div class="container">
        <form action="/api/checkout" method="POST">
            ...
            <h2>04. Início</h2>
            <input type="date" name="data" id="data_select" required onchange="carregarHorarios()" style="color:#fff; background:transparent;">
        ...
        async function carregarHorarios() {
            ...
            for(let i=0; i<24; i++) {
                [":00", ":30"].forEach(m => {
                    ...
                    d.innerText = h;
                    if(!ocupados.includes(h)) d.onclick = () => { document.querySelectorAll('.hora-item').forEach(e => e.classList.remove('selected')); d.classList.add('selected'); document.getElementById('hr_val').value = h; };
...
app.post('/api/checkout', async (req, res) => {
    ...
    const linksAsaas = { "120": "https://www.asaas.com/c/astpmmsj1m8b7wct", "180": "https://www.asaas.com/c/vvznh9nehwe4emft", "240": "https://www.asaas.com/c/1nedgjc1pqqkeu18", "diaria": "https://www.asaas.com/c/9yyhtmtds2u0je33" };
    const linkFinal = (linksAsaas[duracao] || linksAsaas["120"]) + "?externalReference=" + doc;
    try {
        const novaReserva = new Reserva(req.body);
        await novaReserva.save();
        res.send(`<body style="background:#050505; color:#fff; text-align:center; padding-top:150px; font-family:sans-serif;"><h1>ŪNIKA</h1><p>Aguarde...</p><script>setTimeout(() => { window.location.href='${linkFinal}'; }, 2000);</script></body>`);
