...
// 2. ROTA DE WEBHOOK PARA O ASAAS
app.post('/webhook-asaas', async (req, res) => {
    ...
    if (event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_RECEIVED') {
        ...
        try {
            ...
            console.log(`✅ Pagamento confirmado para o CPF: ${cpfCliente}`);
        } catch (err) {
...
<body>
    <header><h1>ŪNIKA</h1><div class="subtitle">Coworking Autônomo • Rio</div></header>
    <div class="container">
        <form action="/api/checkout" method="POST">
            ...
            <div class="agenda-grid" id="ag_container"><p style="grid-column: 1/-1; text-align: center; color: #777; font-size: 1rem; padding: 20px;">Escolha a data acima...</p></div>
            ...
            try {
                ...
                for(let i=0; i<24; i++) {
                    [":00", ":30"].forEach(minuto => {
                        ...
                        d.innerText = h;
                        if(!isOcupado) { d.onclick = () => { document.querySelectorAll('.hora-item').forEach(e => e.classList.remove('selected')); d.classList.add('selected'); document.getElementById('hr_val').value = h; }; }
                        container.appendChild(d);
                    });
                }
            } catch (err) { container.innerHTML = '<p style="color:red; font-size:0.9rem;">Erro na conexão.</p>'; }
        }
    </script>
</body>
</html>`);
    ...
    try {
        ...
        res.send(`
            <body style="background:#050505; color:#fff; text-align:center; padding-top:150px; font-family:sans-serif;">
                ...
                <p style="font-size:1.4rem;">A abrir checkout seguro...</p>
                <script>setTimeout(() => { window.location.href='${linkFinal}'; }, 2000);</script>
            </body>`);
    } catch (err) { res.status(500).send("Erro ao processar reserva."); }
});
